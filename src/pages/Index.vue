...
export default {
  ...
  methods: {
    ...
    getAutoComplete() {
      ...
      this.$axios(`${ this.apiUrlAuto }?q=${this.search}&apiKey=${this.apiKey}&units=metric`)
      .then(response=>{
    ...
    getDataBySearch() {
      this.searchInfo=[]
      this.$axios(`${ this.apiUrl }?at=${this.lat},${this.lon}&q=${this.search}&apiKey=${this.apiKey}&units=metric`)
      .then(response=>{
        ...
        response.data.items.forEach(data=>{
          ...
          this.searchInfo.push(searchData)
        })
        this.lat=this.searchInfo[0].position.lat
        this.lon = this.searchInfo[0].position.lng
        if (this.search) {
          this.$q.localStorage.set("searchName", this.search)
